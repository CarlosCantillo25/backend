...
describe('POST /api/donate', () => {
    it('Debería crear una preferencia ', async () => {
      ...
      const preferenceBody = await request(app).post('/api/donate');
      expect(preferenceBody.status).to.equal(200);
      expect(preferenceBody.body).to.be.an('object');
    });
  });